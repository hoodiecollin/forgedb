...
| `Role` | `name &string`, `permissions json` | `*Organization` |
...
| `AuditLog` | `action`, `resource_type`, `resource_id uuid`, `changes json?`, `occurred_at +timestamp` | `*Organization`, `*User` actor; `@index(resource_type, occurred_at)` |
...
- `json` on `Role.permissions` and `json?` on `AuditLog.changes`: a permission set and a before/after diff have no fixed shape, so they are `serde_json::Value` columns. `json` is deliberately not indexable, filterable or sortable (it has no total order); neither column needs to be, since roles are looked up by name and audit rows by `resource_type` + time

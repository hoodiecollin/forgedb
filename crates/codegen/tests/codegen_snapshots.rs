...
/// A representative per-app C-symbol prefix (#335 §2, decision 9).
///
/// The ONE definition of the VALUE is `forgedb::naming::symbol_prefix`, tested
/// in `src/naming.rs`. `codegen` takes it as a parameter and never derives it,
/// so a literal fixture is the right thing here — and using one that is
/// obviously app-specific is what keeps these assertions from passing against
/// the old constant `forgedb_`.
const SYM: &str = "blog_3f2a1b4c5d6e7f80_";

/// A second app's prefix. Same slug, different member hash — the exact shape
...
"#;
    ...
    for stem in [
        ...
        // Anchored on the DEFINITION token, so a doc comment that still spells
        // the old name cannot make this pass or fail.
        assert!(
...
fn test_ffi_generation_arrow_export() {
    ...
    // `<prefix><m>_<f>_export_arrow` filling the Arrow C-Data-Interface structs.
...
"#;
    ...
    // Not merely "names ColumnExport": naming it ABSOLUTELY (`forgedb_storage::`)
    // would mean a substrate pin of this crate's own, and the two `ColumnExport`
    // types would then unify only by lockfile coincidence.
    assert!(
    ...
    for stem in [
        "user_id_export_arrow(",       // uuid -> w:16
        "user_age_export_arrow(",      // i64  -> l
        "user_score_export_arrow(",    // f64  -> g
        "user_created_at_export_arrow(", // timestamp -> l
        "post_id_export_arrow(",       // uuid -> w:16
        "post_views_export_arrow(",    // i32  -> i
        "post_author_export_arrow(",   // required FK (uuid) -> w:16
    ] {
        assert!(
            flat.contains(&format!("fn{SYM}{stem}")),
...
fn test_go_generation_binding() {
    ...
    // The constant prefix is GONE from the calling side too — go.rs and
    // ffi.rs move together or the binding does not link.
...
///
/// Anchored on the CALL token `C.` rather than on a name pattern, so it keeps
/// finding the calls when the prefix changes — which is exactly the change that
/// would otherwise make the guard silently match nothing.
fn go_c_symbols(go_code: &str) -> Vec<String> {
    ...
    while let Some(pos) = go_code[i..].find("C.") {
        ...
        // `start` is already past the `C.`, so it is both a char boundary and
        // strictly forward progress when the match was not an identifier.
        i = end.max(start);
    }
    out.sort();
    out.dedup();
...
fn test_go_calls_match_ffi_symbols() {
    // Anti-drift guard: every engine symbol the generated Go binding calls MUST
    // be a symbol the FFI generator actually emits for the same schema, AND must
    // be declared in the `forgedb.h` the cgo preamble includes. The Go generator
    // re-derives relation names independently (mirroring the FFI derivation) and
    // now also carries the app's C-symbol prefix, so THREE files have to agree:
    // ffi.rs defines, go.rs's header declares, go.rs's package calls. A mismatch
    // is not a compile error on any of the three — it is an undefined symbol at
    // link time.
    let schema = go_binding_schema();
    ...
    // Only the ENGINE symbols: cgo's own C types (`C.uint8_t`, `C.size_t`,
    // `C.ForgeError`, …) and the two Go-side shims ride the same `C.` syntax.
    ...
    // The one Go-side export is prefixed too: cgo's //export makes it an
    // external C symbol, so two ForgeDB Go packages in one binary collide on it
    // exactly as they would on `open` — and ffi.rs cannot cover it, because Go
    // emits it.
    assert!(
    ...
    // Nothing anywhere may still carry the old constant prefix or an
...
/// Every `#[unsafe(no_mangle)]` C symbol the FFI generator EXPORTS.
///
/// Anchored on the pair (`no_mangle` attribute, `extern "C" fn` definition),
/// never on a name pattern — twice deliberately:
///
/// * a name pattern would have to know the prefix, and a guard that has to be
///   told the answer cannot catch the answer changing;
/// * `arrow_array_release` / `arrow_schema_release` ARE `extern "C"` but are NOT
///   `no_mangle`, so they carry Rust-mangled symbols and cannot collide.
///   Counting them would make the disjointness guard report a collision that
///   does not exist.
...
/// Strip a known prefix off every symbol in a set, asserting each carries it.
///
/// A `strip_prefix(...).unwrap_or(s)` here would quietly pass a symbol that was
/// never prefixed, which is the exact defect this is guarding.
fn symbol_stems(
...
/// Every symbol cgo `//export`s out of the generated Go package. These are
/// external C symbols too — emitted by Go rather than by `ffi.rs`, so the
/// FFI-side prefix does not cover them.
...
/// Two schemas in one project, each declaring a model of the SAME name with the
/// same shape. Every derived symbol stem therefore collides; only the prefix can
/// separate them.
...
/// **Scenario 3 (#335 gate 2, plan #347).** *Given* two schemas in one project
/// each declaring `Post` · *When* both emit `ffi/` · *Then* their `no_mangle`
/// symbol sets are DISJOINT, and a single Go binary importing both links.
///
/// Cargo never sees this class of collision: the two packages have different
/// `[package] name`s, so nothing warns and nothing fails. Under the `cdylib`
/// path it was a load-time collision only if one process loaded both. The
/// `staticlib` Go delivery makes it a **link-time** collision — reachable, and
/// silent until late.
#[test]
fn test_two_apps_export_disjoint_ffi_symbols() {
    ...
    // The fixture must actually exercise the fat surface, or "disjoint" is
    // trivially true of two empty sets.
    assert!(
        sa.len() > 30,
    ...
    // Disjointness must come from the PREFIX, not from the two apps happening to
    // generate different surfaces. Strip each prefix and the sets are equal —
    // which is also what proves the prefix is applied to every symbol rather
    // than to a subset.
    assert_eq!(
        symbol_stems(&sa, SYM),
    ...
    // The stems are the real surface, spot-checked so a scanner that silently
    ...
    // --- the Go half of "a single Go binary importing both links" -----------
    //
    // Each package's own C symbols are the union of what its ffi crate defines
    ...
    // And each package's Go calls must resolve into its OWN ffi symbols — not
    // the sibling's. Calling the wrong app's engine still LINKS, and is the
    // failure a prefix that is merely "different" rather than "threaded from one
...
/// The `ffi/` cache package links `core` and pins nothing else (#335 §1/§6).
#[test]
fn test_ffi_cache_package_manifest() {
    let manifest = FfiGenerator::cargo_toml("blog-3f2a1b4c5d6e7f80-ffi", CORE_PKG);

    // `staticlib` is what Go links; without it there is no `libforgedb.a` to
    // deliver and the whole static path is unreachable.
    assert!(
        manifest.contains(r#"crate-type = ["cdylib", "rlib", "staticlib"]"#),
        "the ffi package must emit a staticlib as well:\n{manifest}"
    );

    // ZERO substrate pins: every substrate type is reached through `core`'s
    // re-exports, which is what makes the types unify rather than merely resolve
    // to the same version.
    for pin in [
    ...
    // A `[profile.*]` table in a workspace MEMBER is silently ignored. The one
    // that used to live here declared `panic = "unwind"` with a comment calling
    // it load-bearing for the catch_unwind boundary — it read as applied and was
    // not. The floor moved to the build driver's cargo invocation.
    assert!(
    ...
    // The MANIFEST carries the hashed package name; the SOURCE must not.
    ...
    // (The C-symbol prefix does carry the app hash, by design — that IS the
    // per-app identity. What must not appear is the cargo package name, which is
    // what the `forgedb_core` rename exists to keep out.)
}

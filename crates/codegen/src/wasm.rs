...
impl WasmGenerator {
    ...
    pub fn generate(schema: &Schema) -> Result<GeneratedCode> {
        ...
        //
        // Routed through `core` rather than reusing `RustGenerator::inline_str_import`
        // (`rust.rs:9753`), which names `forgedb_types` absolutely: this crate pins
        // zero substrate of its own (#335 §1), so every substrate path below goes
        // through `forgedb_core`'s re-exports.  The *predicate* is still the shared
        // one, so the import cannot appear for a schema the database does not emit
        // `InlineStr` for, or go missing for one that does.
        ...
        let tokens = quote! {
            ...
            // The generated database is a SIBLING CRATE in this app's cache
            // workspace (#335 §1), not a module of this one.  `core` is the one
            // emission every wrapper links, which is what makes it impossible for
            // the replica to follow a database that differs from the server's —
            ...
            // Substrate reached through `core`'s re-exports, never pinned here.
            // Two crates that pin `forgedb-storage` independently unify only
            // because one lockfile happened to resolve both pin lists the same
            // way; going through `core` makes agreement a property of the code.
...
"#,
    ...
    /// The `wasm/` cache package's `Cargo.toml` (#335 §1).
    ///
    /// # Zero substrate pins
    ///
    /// The only dependency that can reach ForgeDB substrate is `core`, which
    /// re-exports what this crate names.  A wrapper that pinned
    /// `forgedb-storage` itself would type-unify with `core`'s copy only because
    /// one lockfile happened to resolve two independently-authored pin lists
    /// identically; routing through `core` makes it a property of the code.
    /// Only genuinely non-substrate deps stay here.
    ///
    /// # Nothing in the cache is user-editable
    ///
    /// This is rewritten in full on every generate, unlike the scaffolds in the
    /// output directory, which are written only when absent because they are the
    /// user's.  A carried-forward copy is how a CLI upgrade that bumps a pin
    /// fails to reach an existing project.
    ///
    /// # There is no `[profile.release]`
    ///
    /// It used to carry `opt-level = "s"`.  **A `[profile.*]` table in a
    /// workspace member is silently ignored** (`warning: profiles for the non
    /// root package will be ignored`), so once this package is a member of the
    /// app's cache workspace that block reads as applied and does nothing — the
    /// exact failure mode #333's strict parsing exists to prevent.  The size
    /// floor is applied by the build driver as a `--config` on the `wasm32`
    /// cargo invocation only, because it is a wasm-specific choice and the
    /// workspace root is shared with every native member.
...
fn pk_parse_opt(schema: &Schema, model: &Model) -> TokenStream {
    ...
    match RustGenerator::identity_type(schema, model).as_ref() {
        ...
        // #252: a `string(N)` identity is an `InlineStr<N>`, and the parse is the
        // capacity check — an over-long segment is `None`, i.e. a 404, rather than
        // a truncated id that resolves to somebody else's row.
        //
        // This is the *second* `_ => Uuid`-shaped fall-through on the id path
        // (`api.rs::id_parse_type` was the first), and the one most likely to be
        // forgotten: the replica is not in the default build, so nothing else in
        // the suite exercises it. Left unfixed it returns `None` for every
        // well-formed string key — a replica that silently resolves nothing.
        Some(ty @ FieldType::StringN { .. }) => {
            let key_ty = RustGenerator::key_type_ident(schema, ty);
            quote! { <#key_ty>::try_from(id.as_str()).ok() }
        }
        // A bare `string` identity is refused by validation (#252 res 1), so this
        // arm is unreachable from a valid schema; it stays for the LSP/preview
        // paths, which generate from an unvalidated AST.
...
mod tests {
    ...
    /// #335 §1: the replica links the app's ONE generated database as a crate.
    ///
    /// Anchored on the seam tokens themselves rather than on the word
    /// `database`, which appears in the module docs either way.
    #[test]
    ...
    /// Every substrate path in the body is reached through `core`'s re-exports.
    ///
    /// Asserted on the FULLY-QUALIFIED `use` lines: a bare `forgedb_storage`
    /// substring also matches `use forgedb_core::forgedb_storage::…`, so a
    /// negative check on the short name would fail even when correct, and a
    /// positive one would pass while the absolute import was still present.
    #[test]
    ...
    /// The `InlineStr` import stays conditional, and is core-routed when present.
    #[test]
    ...
    /// The wrapper manifest pins zero substrate — everything comes via `core`.
    #[test]
    fn the_manifest_pins_no_substrate() {
        ...
        // Every substrate crate this used to pin, by its DEPENDENCY-KEY spelling
        // (hyphenated), which `forgedb_core = { package = … }` cannot match.
        for pin in [
            "forgedb-storage",
            "forgedb-types",
            "forgedb-changefeed",
    ...
    /// emitting one would read as applied while doing nothing (#335 gotcha 6).
    /// The size floor moved to the driver's `wasm32` `--config`.
    #[test]

...
- **FOLDED INTO #57 (maintainer decision, 2026-07-07):** the per-model manifest emission + the substrate `Manifest` additive fields (`compaction_epoch`/`format_version`). Implementation revealed that **reopen and compaction both anchor entirely on the tombstone file and consume no manifest** — the manifest's first real reader is #57 backup (archive metadata) / #63 inspector. Building it against that consumer avoids speculative machinery and lets a real reader exercise the column-index/type mapping.

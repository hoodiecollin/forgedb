...
forgedb-codegen       one generator per artifact:
   ...
   └─ TransformGenerator   → transform-<F>-<T>/*     (offline data migration bin)
```
...
- **Durability.** Generated writes journal an opaque row blob to a per-model WAL and fsync under `FsyncPolicy::Always` before touching columns; recovery truncates a torn column tail with `truncate_to_rows` and replays the WAL tail. `open_at` takes a `DirLock` on the directory, and a second writer is refused — the [single-writer contract](/docs/what-pre-1-0-is/). See [durability](/docs/features/durability/) for the full crash-window argument.

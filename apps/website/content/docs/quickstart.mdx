...
`--template` accepts `blog`, `ecommerce`, `todo`, or `blank` (the default). `init` writes:
...
There is no `Cargo.toml` and no `src/main.rs`: ForgeDB compiles the generated Rust in its
own build cache, and the env-driven axum server is one of the artifacts
[`forgedb build`](/docs/cli/build/) produces and reports the path of.
...
ForgeDB compiles the generated code in its own build cache — a cargo workspace under
...
substrate version matrix in [installation](/docs/installation/), and
[`forgedb build`](/docs/cli/build/) for `--plan`, `--report` and `--print-artifact`.

## 5. Run the server

The generated server is an axum binary configured entirely from the environment. Ask
ForgeDB where it is rather than composing the path:

```bash
SERVER="$(forgedb build --print-artifact server)"
FORGEDB_PORT=3000 FORGEDB_DATA="$PWD/data" "$SERVER"
# INFO myblog: ForgeDB serving tenant=None data_root=/…/myblog/data addr=127.0.0.1:3000
```

<Callout type="note" title="Pass an absolute FORGEDB_DATA">

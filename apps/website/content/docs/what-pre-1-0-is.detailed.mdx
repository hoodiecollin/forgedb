...
`cargo install forgedb` builds the CLI from crates.io; prebuilt binaries come from the release workflow; the generated TypeScript SDK is full-CRUD and typed; observability routes, a container deploy path, and a stated semver policy round it out. See [installation](/docs/installation/) and [quickstart](/docs/quickstart/) for the operator path.

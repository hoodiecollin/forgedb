...
Database maintenance and inspection. The read-only `stats` and `analyze` subcommands work
over a data directory; the mutating `run` and `vacuum` subcommands are **deprecated and exit
non-zero**.
...
automatically in-process under the single-writer lock, or you can call `Database::compact()`
from your running app. That path is keep-set-based and never resurrects deletes.
</Callout>
...
Show database-wide statistics as JSON:
...
```bash
forgedb compact run
# error: Offline `forgedb compact` / `vacuum` is REMOVED (issue #105) ...
# (exit code 6)
```

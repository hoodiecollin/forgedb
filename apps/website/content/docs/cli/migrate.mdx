...
Schema evolution follows the generate-then-compile model: you edit `schema.forge`,
regenerate, and rebuild. Additive changes (a new model, a new nullable field) are preserved
automatically on reopen. Everything else rewrites data-at-rest, and `forgedb migrate` drives
that end to end via a generated offline transformer. See the
[migrations feature guide](/docs/features/migrations/) for the full workflow.

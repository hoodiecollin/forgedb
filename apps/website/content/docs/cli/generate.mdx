...
|---|---|---|
| `all` | standalone | **Every** target `[generate].targets` declares — the five always-on artifacts (`rust`, TypeScript SDK, `api`, `openapi`, `stubs`) *and* the opt-in ones: `ffi`, the browser replica, the Python/Rust/Go REST SDKs, and the Node/Bun/Python/Go native runtime bindings. |
...
| `go --runtime` | runtime | cgo in-process binding over the FFI **static archive**. |
...
| `-f`, `--force` | bool (false) | **Accepted and ignored.** Regeneration always overwrites what ForgeDB authored; there is nothing left to force. Kept because scripts and Dockerfiles pass it. |
| `--from <N>` | u32 | Origin format version — `transform` target only. |
| `--to <N>` | u32 | Destination format version — `transform` target only. |

<Callout type="note" title="`ffi` now writes a header, and the native bindings write shims">
`generate ffi` emits `<output>/ffi/forgedb.h` — before, the header was written only on the
Go path and a C consumer had no declarations at all.

`node --runtime` and `python --runtime` emit the consumer-facing half of their binding into
`<output>/napi/` and `<output>/pyo3/`: an `index.js` + `index.d.ts`, and a `forgedb.py` +
`forgedb.pyi`. These are generated text like `types.ts` — commit them, and `--check` covers
them. They load the artifact `forgedb build` delivers beside them and refuse it if it was
built from different generated source, so a stale binding is a named error rather than a
method set that silently no longer matches.

`generate` also writes `<output>/.gitignore`, which ignores the compiled artifacts and
re-includes the shims. The compiled halves are not committed; each machine builds its own.
</Callout>

...
Manage physical, dir-per-tenant data directories. Multi-tenancy is filesystem-enforced:
each tenant's data lives under `<root>/<tenant>/`, and one server process serves exactly one
tenant (process-per-tenant). This command only ever creates, lists, or removes directories —
it never reads a `.forge` schema.

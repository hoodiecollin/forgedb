...
Start the Tier-3 MVCC commit coordinator for a data directory. The coordinator holds the
exclusive directory lock on behalf of all connected clients, serializes the commit turn, and
sequences the commit LSN — enabling multiple *processes* to write to one data directory. See

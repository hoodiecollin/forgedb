...
full CRUD route set; the whole database gets operational and realtime routes. Nothing is
interpreted at runtime — each handler is generated for exactly one schema.
...
| `PUT /api/post/{id}` | Replace the whole record. | `200` / `204` | `404`, `409`, `422` |
...
`total` is the count of matches before pagination.
...
Point your liveness probe at `/health` and readiness at `/ready`. See
[deployment](/docs/reference/deployment/).

## WebSocket routes

Realtime streams. `/subscribe` and `/live-query` sit **behind** the tenant guard;
`/replicate` also sits behind it.
...
forward the `Upgrade`/`Connection` headers so these routes work — see
[deployment](/docs/reference/deployment/).

...
ForgeDB commits to stability across four distinct surfaces, each with its own version line
and its own rules.
...
**Breaking (major bump only):** removing/renaming a type, directive, modifier, or field;
...
gate enforces at the *data* level — the schema-language policy and the data-migration
policy agree on what "breaking" means.
...
- The CLI that *generates* against a substrate version and the substrate move together —
  `forgedb init` always pins versions the current CLI is known-compatible with.
...
Until then, see [what v1 is](/docs/what-v1-is/) for the honest current state.
...
- [What v1 is](/docs/what-v1-is/) — the honest scope behind these promises.

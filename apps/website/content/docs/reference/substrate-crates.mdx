...
Every public item of the host-side crates is documented on docs.rs (`https://docs.rs/<crate>`).
ForgeDB's own source carries no comments, so that prose lives in per-item sidecar files the
build includes at docs.rs time; a public item without one fails CI. The `forgedb-storage` page
documents only the facade and links `forgedb-storage-native` for the items. The sidecars carry

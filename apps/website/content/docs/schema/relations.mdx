...
type, `timestamp`, or [`string(N)`](/docs/schema/scalar-types/#stringn-as-an-identity) —
the last of which qualifies because a key of that type is a `Copy`, fixed-capacity
`InlineStr<N>` rather than a heap `String`. Anything else is reported as a schema error.

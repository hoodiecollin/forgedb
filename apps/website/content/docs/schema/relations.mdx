...
Relations connect models. ForgeDB generates the persisted foreign-key columns, the
junction tables, and typed traversal methods (forward, reverse, and eager-load) from the
relation syntax below.
...
When **both** models list the other with a `[OtherModel]` field (written `[..]` in the table
above) and neither side is a foreign key, the parser auto-detects a many-to-many relationship

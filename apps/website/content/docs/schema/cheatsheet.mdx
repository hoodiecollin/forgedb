...
|-------|----------|---------|-------|
| `+` | prefix | auto-generate | `u32`/`u64`/`uuid`/`timestamp` only — all fill on create when omitted. Integer autos are monotonic, **not gapless**, cannot take an explicit `0`, and must be the identity or `&unique` |

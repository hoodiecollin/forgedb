...
| `string(N)` `string(N!)` | `String` | `string` | ✅ — fixed N-**character** slot in the row; `!` = exactly N, bare = at most. ASCII unless `@utf8` |
...
|-------|----------|---------|-------|
| `+` | prefix | auto-generate | `u32`/`u64`/`uuid`/`timestamp` only — all fill on create when omitted. Integer autos are monotonic, **not gapless**, and cannot take an explicit `0`; any shape (identity, `&`, `^`, or bare) is valid |
...
A foreign key takes its target's identity type. See [relations](/docs/schema/relations/).

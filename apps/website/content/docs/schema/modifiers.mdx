...
<Callout type="warning" title="An integer auto must be the identity or `&unique`">
A bare non-unique `seq: +u64` is a **fatal** validation error. Mark it `&`, or make it the
model's identity.
...
silent is the write-set the coordinator compares — and only the identity field and a
`&unique` field put a claim there. `^` alone is not enough: an index makes a value fast to
find, but claims nothing at commit time, so both writers would commit and neither would
notice.

```forge
Invoice {
  id: +uuid
  number: &+u64   // non-identity integer auto — the `&` is required
}
```
</Callout>
